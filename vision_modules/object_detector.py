...
class YOLOv8Detector(Detector):
    ...
    # 识别瓶子和容器
    ALLOWED_CLASS_IDS = {39, 41, 46}  # 39=bottle(瓶子), 41=cup(杯/罐), 46=wine glass
    ...
    def __init__(self, 
        ...
        Args:
            ...
            nms_iou_threshold: NMS IoU阈值，控制框的重叠程度（越小越不过滤）
    ...
    def detect(self, frame: FrameData) -> List[DetectionResult]:
        ...
        try:
            # 运行检测，使用NMS IoU阈值控制框的合并
            results = self.model(frame.image, conf=self.confidence_threshold, iou=self.nms_iou_threshold, verbose=False)
            ...
            if results and len(results) > 0:
                ...
                if result.boxes is not None:
                    ...
                    for i, box in enumerate(boxes):
                        ...
                        # 打印所有检测到的物体
                        print(f"[DEBUG] Detected: class_id={class_id}, class_name={class_name}, confidence={confidence:.2f}")
